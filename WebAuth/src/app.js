...
Amplify.configure({
    Auth: {
        ...
        userPoolId: 'eu-west-1_NY5VYKPpl',

        // OPTIONAL - Amazon Cognito Web Client ID (26-char alphanumeric string)
        userPoolWebClientId: '6108icgldqvm5l3g0shohh9cav',
        ...
        endpoints: [
            {
                ...
                endpoint: "https://tihgps0zh8.execute-api.eu-west-1.amazonaws.com"
            }
        ]
    }
});
...
RequestToSecuredEndpointButton.addEventListener('click', async evt => {
  ...
  await API.get("MyAPIGatewayAPI", '/dev/api/cognitoPrivate', myInit).then(response => {
...
RequestToNonSecuredEndpointButton.addEventListener('click', async evt => {
  await API.get("MyAPIGatewayAPI", '/dev/api/cognitoPublic').then(response => {
    console.log(response);
  }).catch(error => {
      console.log(JSON.stringify(error))
  });
